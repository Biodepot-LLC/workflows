...
class OWstarAlign(OWBwBWidget):
    ...
    inputs = [("trigger",str,"handleInputstrigger"),("FastQTrimmedFiles",str,"handleInputsFastQTrimmedFiles"),("StarIndexDir",str,"handleInputsStarIndexDir")]
    ...
    def handleInputsFastQTrimmedFiles(self, value, *args):
        if args and len(args) > 0: 
            self.handleInputs("FastQTrimmedFiles", value, args[0][0], test=args[0][3])
        else:
            self.handleInputs("inputFile", value, None, False)
    def handleInputsStarIndexDir(self, value, *args):
        if args and len(args) > 0: 
            self.handleInputs("StarIndexDir", value, args[0][0], test=args[0][3])
        else:
            self.handleInputs("inputFile", value, None, False)
