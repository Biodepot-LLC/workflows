...
parms=$@
echo "nice runUploadSplitFiles.sh $CREDENTIALS $OUTPUTDIR $NFILES gs://${BUCKET}/${CLOUD_DIR}/ $UPLOAD_THREADS $UPLOAD_THREADS_AFTER"
nice runUploadSplitFiles.sh $CREDENTIALS $OUTPUTDIR $NFILES gs://${BUCKET}/${CLOUD_DIR}/ $UPLOAD_THREADS $UPLOAD_THREADS_AFTER &
upload_pid=$!
